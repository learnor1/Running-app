...
if (!Array) {
  ...
  (_component_van_sidebar_item + _component_van_sidebar + _component_van_card + _component_van_button + _component_van_empty + _component_van_tab + _component_van_tabs + _component_van_overlay + _component_van_loading)();
}
const _sfc_main = {
  __name: "order",
  setup(__props) {
      ...
      if (jieliu1) {
        await request_index.request({
          ...
          if (res.data.data.length == 0) {
            ...
            console.log(foodsPublishList.value);
    ...
    const contactPao = async (cartId) => {
      await request_index.request({
        url: `feedOrderCenter/getRunningUser/ByCartId?cartId=${cartId}`,
        method: "POST"
      }).then((res) => {
        common_vendor.index.navigateTo({
          url: `/subpages/conversation/conversation?otherId=${res.data.data}`
        });
      });
    };
    const dateTime = (i) => {
      ...
      return {
        ...
        f: common_vendor.f(foodsPublishList.value, (i, index, i0) => {
          return common_vendor.e({
            ...
            f: common_vendor.o(($event) => contactPao(i.cartId), index),
            g: "10c9564e-6-" + i0 + ",10c9564e-1",
            h: common_vendor.p({
              type: "primary",
              size: "small"
            })
        ...
        j: common_vendor.f(otherPublishList.value, (i, index, i0) => {
          return common_vendor.e({
            ...
            f: common_vendor.o(($event) => contactPao(i.cartId), index),
            g: "10c9564e-9-" + i0 + ",10c9564e-1",
            h: common_vendor.p({
              type: "primary",
              size: "small"
            })
          } : {
            i: common_vendor.t(common_vendor.unref(otherState)[i.order_state])
          }, {
            j: common_vendor.o(($event) => goOtherDetail(i.content, i.order_state), index),
            k: index
          });
        }),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "D:/frontendcode/project/wx-Running/wx-Running-master/pages/order/order.vue"]]);
